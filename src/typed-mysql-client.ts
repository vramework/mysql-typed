...
import { createBulkInsert, createInsert, exactlyOneResult, getFilters, Logger, QueryInterface, transformValues, ValueTypes } from './database-utils'

export class TypedMySQLClient<Tables extends { [key: string]: any }, CustomTypes> {
  private client: PoolConnection | null = null

  constructor(private pool: TypedMySQLPool<Tables, {}>, private logger: Logger) {
  }
  ...
  public async crudDelete<N extends keyof Tables, T extends Tables[N], F extends readonly (keyof T)[]>(table: N, filters: Partial<T>, returns: F = [] as unknown as F, notSingleError?: Error): Promise<void | Pick<T, typeof returns[number]> | Pick<T, typeof returns[number]>[]> {
    const { filter, filterValues } = getFilters(filters)
    if (returns.length > 1) {
      const result = await this.query<Pick<T, typeof returns[number]>>(`DELETE FROM "app"."${table}" RETURNING ${returns.join(',')}, ${filter}`, filterValues)
      if (notSingleError) {
        return exactlyOneResult(result.rows, notSingleError)
      }
      return result.rows
  ...
  public async debugQuery<T>(
    ...
    const result = await this.query<T>(statement, values, 'debug')
    return result.rows
  }
  ...
  public async query<T>(
    ...
    try {
      const [rows, meta] = await this.client?.query<RowDataPacket[]>(query, values)
